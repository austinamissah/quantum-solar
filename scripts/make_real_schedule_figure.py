...
MID_LO, MID_HI = 10, 20        # "mid-month" window, by day-of-month

# One representative day per seasonal bucket. Summer is Jun-Sep (the RE-TOU
# tariff season); the month is a target and pick_day resolves it to a real
# AMY-2018 calendar day of the right weekday/weekend type.
BUCKETS = (
...
_GENERATION: np.ndarray | None = None  # set in main(); read by pick_day


def pick_day(month: int, wanted: str) -> int:
    """A representative AMY-2018 day: mid-month, right day type, not a holiday.

    Two deliberate choices, both to keep the figure representative rather than
    incidental:
    ...
    * **Mid-month, and typical within it.** The first matching day of a month sits
      at a season boundary where generation is least typical of the month it
      labels. But "closest to the 15th" alone is not enough either: in this
      snapshot it selects a July day at the **3rd percentile** of July generation
      (16.1 kWh against a 23.2 kWh median) -- a heavily overcast day, which is a
      misleading thing to publish as "a summer day". So the pick is the day whose
      total generation is nearest the **median** of its mid-month window. Median,
      not maximum: the goal is a typical day, not a flattering one.
    """
    window = [d for d in range(365)
              if day_to_month(d) == month
              and day_type(d) == wanted
              and not is_federal_holiday(d)
              and MID_LO <= day_of_month(d) <= MID_HI]
    if not window:
        raise ValueError(f"no non-holiday mid-month {wanted} in month {month}")
    totals = {d: float(to_slots(_GENERATION, d, 24).sum()) for d in window}
    target = float(np.median(list(totals.values())))
    return min(window, key=lambda d: (abs(totals[d] - target), d))  # ties -> earlier
...
def main():
    ...
    _GENERATION = np.asarray(snap["generation"], dtype=float)
    record = {
