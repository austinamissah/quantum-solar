...
| [hardware-run.md](plans/hardware-run.md) | H1: at each size, reps=1 hardware optimal mass is at least reps=2's; refuted, the plan says, if reps=2 exceeds reps=1 | reps=1 won at 2 slots and reps=2 at 3 slots, which is the stated refutation; the notebook's first reading, "partially supported at best", is corrected in place; the 3-slot counts sit at the shot floor (1 and 9 of 4096) and the ordering mirrors the ideal one, which bounds what the result says about depth | **falsified** | [experiment_hardware.ipynb](../notebooks/experiment_hardware.ipynb) |
...
condition, which the notebook first recorded as "partially supported at best" and
now corrects in place. The test
